...
export default function SettingsScreen() {
  ...
  const handleTaskReminders = async (next: boolean) => {
    if (next) {
      ...
      await scheduleTaskReminderSample();
    ...
    if (next) {
      ...
      if (permission !== "granted") {
        ...
        if (result !== "granted") {
          Alert.alert(
            "Notifications",
            "Notification permission is required for Sabbath Pause.",
          );
          return;
        }
      }
  ...
  const rateApp = async () => {
    ...
    Alert.alert(
      "Rate This App",
      "Thank you for your support! Please rate us on the App Store when the app is published.",
    );
  };
  ...
  const showProThemes = () => {
    Alert.alert(
      "Custom Themes",
      "Custom themes are included with Pro. Tap Buy Pro Version Now! to unlock.",
    );
  };
...
function PremiumCard({ onPress }: { onPress: () => void }) {
  return (
    <Pressable
      ...
      accessibilityLabel="Buy Pro Version Now"
    >
      ...
      <View style={styles.premiumMiddle}>
        <Text style={styles.premiumTitle}>Buy Pro Version Now!</Text>
